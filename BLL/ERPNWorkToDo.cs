...
namespace ZWL.BLL
{
	...
	public class ERPNWorkToDo
	{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,WorkName,WenHao,FormID,WorkFlowID,UserName,TimeStr,FormContent,FuJianList,ShenPiYiJian,JieDianID,JieDianName,ShenPiUserList,OKUserList,StateNow,LateTime ");
			...
			if(ds.Tables[0].Rows.Count>0)
			{
                ...
                WenHao = ds.Tables[0].Rows[0]["WenHao"].ToString();
		...
		public int Add()
		{
			...
			strSql.Append("WorkName,FormID,WorkFlowID,UserName,TimeStr,FormContent,FuJianList,ShenPiYiJian,JieDianID,JieDianName,ShenPiUserList,OKUserList,StateNow,LateTime,WenHao)");
		...
		public void Update()
		{
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@StateNow", SqlDbType.VarChar,50),
					new SqlParameter("@LateTime", SqlDbType.DateTime),
                    new SqlParameter("@WenHao", SqlDbType.VarChar,50)};
			parameters[0].Value = ID;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,WorkName,FormID,WorkFlowID,UserName,TimeStr,FormContent,FuJianList,ShenPiYiJian,JieDianID,JieDianName,ShenPiUserList,OKUserList,StateNow,LateTime,WenHao ");
			strSql.Append(" FROM ERPNWorkToDo ");
			strSql.Append(" where ID=@ID ");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)};
        ...
        /// 获得数据列表
        /// </summary>
        public DataSet GetListUnion(string strWhere1,string strWhere2)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" select * from");
            strSql.Append(" (");
            strSql.Append(" select a.*  from erpnworktodo  a left join erpnworkflowtodouser b on a.id = b.todoid");
            if (strWhere1.Trim() != "")
            {
            ...
            strSql.Append(" ) c order by ID desc");
            return DbHelperSQL.Query(strSql.ToString());
        }
